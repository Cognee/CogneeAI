...
```
sensor.js [16 сенсоров] → CogneeAI ONNX [MLP] → КИМ [0-100]
         ↓                                                    ↓
...
| AI текст | Gemini 2.0 Flash | Бесплатный тариф Google AI (лимиты RPM/RPD периодически меняются — актуальные значения см. на ai.google.dev) |
...
├── moderation.html      ← Панель модерации жалоб на статьи
├── sdk-demo.html        ← Демо-страница внедрения cogneeai.js на сторонний сайт
├── sensor.js            ← 16 поведенческих сенсоров + ONNX-инференс
├── adapter.js           ← Движок адаптации: режимы, «Объясни иначе», закладки, хронорежим
├── article-actions.js   ← Действия над статьёй: публикация, шаринг, избранное
...
├── config.js             ← Публичные ключи (anon-ключ Supabase; ключ Gemini на сервере)
├── favicon/              ← Иконки сайта
└── model/
    └── cognee_ai.onnx    ← CogneeAI MLP-модель
```
...
**Архитектура:** MLP — Dense(64) → Dense(32) → Softmax(5), вход без временнóй развёртки (не RNN)  
**Входные данные:** 16 поведенческих признаков (нормализованы 0–1), тензор формы `[1, 16]`  
...
- **Умные закладки** — сохраняют позицию остановки, баннер-напоминание при возврате в статью
- **Хронорежим** — деликатные уведомления в ночное время и послеобеденный спад
- **Панель модерации** — рассмотрение жалоб на публикации в каталоге
